...
- **🤖 AI 策展** — 3-Stage Pipeline：评分 → 精选 → 质量审查，每阶段独立 context window，质量门禁不通过自动退回重选
...
└──────────┬──────────┘
           ▼
┌─────────────────────┐
│  curator.ts          │  3-Stage AI Pipeline
│  ┌─────────────────┐│
│  │ ① Score         ││  6维评分 — 选题·内容·深度·实用·创新·表达
│  ├─────────────────┤│
│  │ ② Select        ││  精选 3🔥 + 7📰 中文摘要 + 入选理由
│  ├─────────────────┤│
│  │ ③ Review        ││  质量门禁 · 不过关退回重选 (最多3轮)
│  └──────┬──────────┘│
└─────────┼───────────┘
          ▼
┌─────────────────────┐
...
│   ├── curator.ts        # AI 策展 3-Stage Pipeline (Score→Select→Review)
